...
//! See data-model.md for complete specifications and relationships.
...
// ============================================================================
// Supporting Enums (FR-006, data-model.md lines 216-252)
// ============================================================================

/// Order flow direction indicator based on bid/ask flow ratio (FR-006)
///
/// Thresholds:
...
impl FlowDirection {
    /// Determine flow direction from bid/ask flow rates (FR-006)
...
///
/// Mapping (data-model.md lines 227-234):
...
/// Expected price movement impact through liquidity vacuum (FR-008)
///
/// Thresholds (data-model.md lines 237-244):
...
// ============================================================================
// Core Entities (data-model.md lines 25-210)
// ============================================================================

/// Order flow snapshot over configurable time window (FR-001, data-model.md lines 27-51)
///
/// Tracks buying/selling pressure dynamics via bid/ask order addition/cancellation rates.
/// No individual order ID tracking - aggregated counts only (clarification Q4).
...
pub struct OrderFlowSnapshot {
    ...
    /// Categorical pressure indicator (FR-006)
...
/// Volume profile histogram with POC/VAH/VAL (FR-002, data-model.md lines 54-90)
///
/// Shows volume distribution across price levels using adaptive tick-based binning
/// (clarification Q1: `bin_size = max(tick_size × 10, price_range / 100)`).
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
...
/// Market microstructure anomaly (FR-003, FR-004, FR-005, data-model.md lines 93-141)
///
...
/// Anomaly type variants (data-model.md lines 110-128)
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(tag = "type")]
...
/// Liquidity vacuum (FR-008, data-model.md lines 144-168)
///
...
/// Absorption event (FR-009, data-model.md lines 171-210)
///
...
/// Entity type classification for absorption events (data-model.md lines 188-195)
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
pub enum EntityType {
